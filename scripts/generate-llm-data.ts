...
function hasValidPricing(data: ModelData): boolean {
    ...
    const hasTokenPricing = isValidPositiveNumber(data.input_cost_per_token) || isValidPositiveNumber(data.output_cost_per_token);
    ...
    const inputCostPer1M = data.input_cost_per_token
        ? data.input_cost_per_token * 1000000
        : 0;
    const outputCostPer1M = data.output_cost_per_token
        ? data.output_cost_per_token * 1000000
        : 0;

    // If both costs exist but both are below threshold, discard
    if (isValidPositiveNumber(data.input_cost_per_token) &&
    ...
    if (isValidPositiveNumber(data.output_cost_per_token) &&
...
async function main() {
    ...
    const providers = Array.from(new Set(processedModels.map(m => m.provider))).filter(p => p).sort();
    console.log(`Found ${providers.length} unique providers`);

    // Get unique modes from valid models only
    const modes = Array.from(new Set(processedModels.map(m => m.data.mode))).filter(m => m && m.trim() !== '').sort();
