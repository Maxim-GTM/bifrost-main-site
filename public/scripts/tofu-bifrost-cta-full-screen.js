(function() {
  ...
  // Randomly selected on every page load/refresh
  // ============================================================
  ...
  var COHORTS = [
    {
      ...
      primaryBtn: { 
        text: 'Book a Demo', 
        baseUrl: 'https://www.getmaxim.ai/bifrost/book-a-demo'
      }
    },
    {
      id: 'enterprise',
      ...
      primaryBtn: { 
        ...
        baseUrl: 'https://www.getmaxim.ai/bifrost/enterprise'
      }
    },
    {
      id: 'mcp',
      ...
      primaryBtn: { 
        text: 'Learn More', 
        baseUrl: 'https://www.getmaxim.ai/bifrost/resources/mcp-gateway'
      }
    },
    {
      id: 'claude-code',
      ...
      primaryBtn: { 
        ...
        baseUrl: 'https://www.getmaxim.ai/bifrost/resources/claude-code'
      }
    },
    {
      id: 'benchmarks',
      ...
      primaryBtn: { 
        ...
        baseUrl: 'https://www.getmaxim.ai/bifrost/resources/benchmarks'
      }
    }
  ];
  ...
  var SETTINGS = {
    ...
    cookieName: 'bifrost_tofu_dismissed',
    cookieExpireDays: 1           // Don't show for 1 day after user closes
  };
  ...
  // Cookie helpers for dismissal tracking
  ...
  // Build UTM parameters dynamically based on current page
  function getUtmSource() {
    var pathname = window.location.pathname;
    // Remove leading/trailing slashes and replace remaining slashes with underscores
    var source = pathname.replace(/^\/|\/$/g, '').replace(/\//g, '_');
    return source || 'homepage';
  }

  function buildUrlWithUtm(baseUrl, cohortId) {
    var utmSource = getUtmSource();
    var utmMedium = 'cta_modal';
    var utmCampaign = cohortId;
  ...
  function injectStyles() {
    if (document.getElementById('bifrost-tofu-cta-styles')) return;
    var style = document.createElement('style');
    style.id = 'bifrost-tofu-cta-styles';
  ...
  function createModal(cohort) {
    ...
    var existing = document.getElementById('bifrost-tofu-cta-overlay');
    if (existing) {
      existing.remove();
    }

    // Build URL with dynamic UTM parameters
    var buttonUrl = buildUrlWithUtm(cohort.primaryBtn.baseUrl, cohort.id);

    var overlay = document.createElement('div');
    overlay.id = 'bifrost-tofu-cta-overlay';
    ...
    var html = '<div class="bifrost-modal">' +
      ...
      '<div class="bifrost-modal-content">' +
        ...
        '<div class="bifrost-buttons-section">' +
          '<a href="' + buttonUrl + '" target="_blank" rel="noopener noreferrer" class="bifrost-primary-btn">' +
    ...
    // Track click
    var primaryBtnEl = overlay.querySelector('.bifrost-primary-btn');
    if (primaryBtnEl) {
      primaryBtnEl.addEventListener('click', function() {
        if (window.dataLayer) {
          window.dataLayer.push({
            'event': 'bifrost_tofu_modal_click',
            'cohort_id': cohort.id,
  ...
  function showModal() {
    ...
    var overlay = document.getElementById('bifrost-tofu-cta-overlay');
    if (overlay) {
  ...
  function init() {
    if (isDismissed()) {
      console.log('Bifrost TOFU CTA Modal: Dismissed, will show again after cookie expires');
      return;
    }

    var cohort = getCohort();
    console.log('Bifrost TOFU CTA Modal: Selected cohort:', cohort.id);
