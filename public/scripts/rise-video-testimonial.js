(function() {
  ...
  const VIDEO_URL = 'https://maxim-cdn.t3.storage.dev/Video_Assets/How_Product_Managers_at_Rise_Science_Drive_AI_Evals_Case_Study_1080P.mp4';
  const VIDEO_THUMBNAIL = 'https://maxim-cdn.t3.storage.dev/Image_Assets/rise_science_thumnail.png'; // Replace with your thumbnail URL
  
  // Testimonial banner configuration for each tag
  const TESTIMONIAL_CONFIG = {
    'observability': {
      ...
      author: 'Kellie (Kuzmuk) Maloney',
      authorTitle: 'Product Lead',
      companyName: 'Rise Science',
      companyUrl: 'https://www.risescience.com/',
      videoIntro: 'Watch How Product Managers at Rise Science Drive AI Evals with Maxim AI',
      videoUrl: VIDEO_URL,
  ...
  function insertTestimonialBanner() {
    ...
    if (!articleTag || !TESTIMONIAL_CONFIG[articleTag]) {
      console.log('Maxim Testimonial: No matching tag found or banner not configured');
      return;
    }
    
    const config = TESTIMONIAL_CONFIG[articleTag];
    ...
    const testimonialBanner = createTestimonialBanner(config, articleTag);
    firstParagraph.parentNode.insertBefore(testimonialBanner, firstParagraph.nextSibling);
    console.log(`Maxim Testimonial: Inserted ${articleTag} banner after paragraph following "${targetHeading.textContent.trim()}"`);
  }
