...
async function handler(): Promise<NextResponse> {
  try {
    const response = await fetch('https://bifrost-articles.ghost.io/sitemap-authors.xml', {
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch sitemap-authors.xml: ${response.status}`)
    }
    ...
    return new NextResponse('Error fetching sitemap-authors.xml', { status: 500 })
  }
}
