...
async function handler(): Promise<NextResponse> {
  try {
    const response = await fetch('https://bifrost-articles.ghost.io/sitemap-posts.xml', {
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch sitemap-posts.xml: ${response.status}`)
    }
    ...
    console.error('Error fetching sitemap-posts.xml:', error)
    return new NextResponse('Error fetching sitemap-posts.xml', { status: 500 })
  }
}
