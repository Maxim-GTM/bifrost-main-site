...
export async function submitPartnerApplication(formData: FormData) {
    try {
        ...
        if (!data.firstName || !data.lastName || !data.workEmail || !data.companyName || !data.companySize || !data.companyHQ || !data.partnerType || !data.message) {
        ...
        return { success: true, message: 'Application submitted successfully! Our partnerships team will be in touch within 2 business days.' };
    } catch (error: unknown) {
