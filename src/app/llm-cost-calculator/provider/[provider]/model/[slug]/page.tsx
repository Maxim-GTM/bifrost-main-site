...
function detectSpecialization(params: {
  ...
  if (/\btts\b|text[-\s]?to[-\s]?speech|speech synthesis|synthesize|voice/.test(name))
    return 'audio_speech'
  if (/\btranscrib|\btranscription\b|speech[-\s]?to[-\s]?text|\bstt\b|\bwhisper\b/.test(name))
    return 'audio_transcription'
  if (/\bimage\b|stable[-\s]?diffusion|\bsd\b|dall[-\s]?e|imagen|flux/.test(name))
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = buildCanonicalUrl(
    `${getCostCalculatorBaseUrl()}/llm-cost-calculator/provider/${encodeURIComponent(decodedProvider)}/model/${slug}`
  )
  ...
  if (model.data.input_cost_per_token != null) {
    priceParts.push(
      `Input: $${(model.data.input_cost_per_token * 1_000_000).toFixed(2)} per 1M tokens`
    )
  }
  if (model.data.output_cost_per_token != null) {
    priceParts.push(
      `Output: $${(model.data.output_cost_per_token * 1_000_000).toFixed(2)} per 1M tokens`
    )
  }
  ...
  if (model.data.output_cost_per_second != null) {
    priceParts.push(
      `Output audio/video: $${model.data.output_cost_per_second.toFixed(4)} per second`
    )
  }
...
function generateContentData(model: ReturnType<typeof getModelBySlug>) {
  ...
  const isInputCheap = data.input_cost_per_token && data.input_cost_per_token < 0.000002
  const isMultimodal =
    data.supports_vision || data.supports_audio_input || data.supports_audio_output
  ...
  if (signals.isAudioSpeech) {
    ...
    recommendedUseCases.push(
      'Automated video content pipelines where duration-based pricing matters'
    )
  ...
  if (signals.isAudioSpeech) {
    ...
    comparisonText =
      'This is a reranking model. Compared to general-purpose generators, rerankers are optimized for ordering candidates by relevance. When comparing similar models, evaluate relevance quality on your retrieval set and how pricing scales with the number and length of candidates.'
  } else if (signals.isImageGeneration) {
    comparisonText =
    ...
    comparisonText =
      'This is a video generation model. Compared to text-only models, pricing commonly scales with seconds of output video (and sometimes prompt tokens). When comparing alternatives, consider output quality, latency, and how duration-based pricing affects your workload.'
  } else if (signals.isOCR) {
    comparisonText =
      'This is an OCR-focused model. Compared to general-purpose models, OCR pricing often scales per page or per image. When comparing alternatives, evaluate extraction quality on your document types and how costs scale with page volume.'
    ...
    comparisonText =
      'This model combines tool-calling capabilities with advanced reasoning, making it well-suited for complex agentic workflows that require both function execution and strategic thinking.'
  } else {
    comparisonText =
...
export default async function ModelPage({ params }: PageProps) {
  ...
  const hasLimits =
    model.data.max_input_tokens || model.data.max_output_tokens || model.data.max_tokens
