...
import ModelCompare from '@/components/model-library/ModelCompare'
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  if (typeof compareParam === 'string') {
    ...
    if (provider2 && slug2) {
      secondModel = getModelBySlug(
        modelsData,
        decodeURIComponent(slug2),
        decodeURIComponent(provider2)
      )
    }
  }
  ...
  if (secondModel) {
    const canonical = buildCanonicalUrl(
      `/compare/${encodeURIComponent(model.provider)}/${model.slug}?compare=${encodeURIComponent(secondModel.provider)}/${secondModel.slug}`
    )
    return {
  ...
  const canonical = buildCanonicalUrl(
    `/compare/${encodeURIComponent(model.provider)}/${model.slug}`
  )
