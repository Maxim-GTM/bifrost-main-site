...
export function processModels(
  ...
  return Object.entries(models)
    .map(([id, data]) => {
      ...
      const hasInputCost =
        normalizedData.input_cost_per_token != null ||
        normalizedData.input_cost_per_image != null ||
        normalizedData.input_cost_per_second != null
      const hasOutputCost =
        normalizedData.output_cost_per_token != null ||
        normalizedData.output_cost_per_image != null ||
        normalizedData.output_cost_per_second != null
      const hasPricing = hasInputCost || hasOutputCost
      const hasInputTokens =
        normalizedData.max_input_tokens != null && normalizedData.max_input_tokens > 0
      const hasOutputTokens =
        normalizedData.max_output_tokens != null && normalizedData.max_output_tokens > 0
      const hasContextLength =
        normalizedData.context_length != null && normalizedData.context_length > 0
...
export function getModelsByDisplayName(models: ModelEntry, displayName: string): ProcessedModel[] {
  const processed = processModels(models)
  return processed.filter((model) => model.displayName.toLowerCase() === displayName.toLowerCase())
}

export function getModelsByProvider(models: ModelEntry, provider: string): ProcessedModel[] {
  return processModels(models).filter(
...
export function getModelsByMode(models: ModelEntry, mode: string): ProcessedModel[] {
  return processModels(models).filter((model) => model.data.mode === mode)
}
