...
export default function StatusCTA({ providerName }: StatusCTAProps) {
  ...
  const description = providerName
    ...
    : 'Bifrost is an open-source AI gateway that automatically fails over to healthy providers when outages hit. Zero code changes. 99.999% effective uptime.'
  ...
  return (
    ...
    >
      {/* Corner accents */}
      <div className="absolute top-0 left-0 h-2 w-2 -translate-x-0.5 -translate-y-0.5 border-t border-l border-gray-300" />
      <div className="absolute top-0 right-0 h-2 w-2 translate-x-0.5 -translate-y-0.5 border-t border-r border-gray-300" />
      <div className="absolute bottom-0 left-0 h-2 w-2 -translate-x-0.5 translate-y-0.5 border-b border-l border-gray-300" />
      <div className="absolute right-0 bottom-0 h-2 w-2 translate-x-0.5 translate-y-0.5 border-r border-b border-gray-300" />
      ...
      <div className="relative flex h-full min-h-0 flex-1 flex-col justify-between gap-10 p-8 lg:p-10">
        ...
        <div className="flex shrink-0 flex-col items-center space-y-3">
          ...
          <div className="flex items-center justify-center">
            <div className="text-accent inline-flex items-center justify-center font-mono text-xs leading-none font-medium">
              [ AUTOMATIC FAILOVER ]
            </div>
          </div>
        </div>
        ...
        <div className="shrink-0 space-y-4">
          <h3 className="text-center text-xl leading-tight font-bold text-gray-900 sm:text-2xl lg:text-3xl">
            {heading.split('Route around it.').length > 1 ? (
              <>
                {heading.split('Route around it.')[0]}
                <span className="text-accent">Route around it.</span>
              </>
            ) : (
              <>
                Never go down{' '}
          ...
          <p className="mx-auto max-w-lg text-center text-sm leading-relaxed text-gray-600 lg:text-base">
            {description}
          </p>
        </div>
        ...
        <div className="flex shrink-0 flex-col items-center justify-center gap-3">
          ...
          <Link
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="items-center justify-center text-xs text-black"
          >
            or,{' '}
            <span className="underline decoration-[#35c09e] underline-offset-2">
              Get started here
            </span>
          </Link>
        </div>
