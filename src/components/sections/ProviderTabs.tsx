...
response = client.chat.completions.create(
    ...
    },
    {
      ...
      icon: (
        <div className="">
          <svg width="22" viewBox="0 0 46 32" fill="none">
...
response = client.models.generate_content(
    ...
    contents="Hello!"
      ...
      bifrostLine:
        '    http_options=HttpOptions(base_url="https://<your_bifrost_deployment_base_url>/genai")',
    },
  ]
  ...
  return (
    <div
      ref={containerRef}
      className="animate-fade-in-up relative mx-auto max-w-4xl overflow-hidden rounded-md border border-[#3c3c3c] shadow-lg delay-300"
    >
      {/* VSCode-like Tab Bar */}
      <div className="flex items-center justify-between overflow-x-auto border-b border-[#3c3c3c] bg-[#2d2d2d] text-xs">
        <div className="flex overflow-x-auto">
          {providers.map((provider) => (
            <button
              key={provider.id}
              onClick={() => setActiveTab(provider.id)}
              className={`flex items-center gap-2 border-r border-[#3c3c3c] px-6 py-2 whitespace-nowrap transition-colors duration-200 ${
                activeTab === provider.id
                  ? 'bg-[#1e1e1e] text-white'
                  : 'bg-[#2d2d2d] text-gray-400 hover:bg-[#262626] hover:text-white'
              }`}
              style={{ height: '38px' }}
            >
              {provider.icon}
        ...
        <button
          ...
          className="mx-2 flex cursor-pointer items-center gap-1 rounded border border-[#4c4c4c] bg-[#3c3c3c] px-3 py-1 text-xs text-gray-400 transition-colors duration-200 hover:bg-[#4c4c4c] hover:text-white"
        ...
        {showButton && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <button
              ...
              className="group pointer-events-auto relative transform cursor-pointer overflow-hidden rounded-sm bg-gradient-to-r from-green-500 to-green-600 px-8 py-4 text-lg font-semibold text-white shadow-2xl transition-all duration-300 hover:scale-105 hover:from-green-600 hover:to-green-700"
              style={{
                background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                boxShadow:
                  '0 0 40px rgba(16, 185, 129, 0.4), 0 0 80px rgba(16, 185, 129, 0.2), 0 10px 30px rgba(0, 0, 0, 0.3)',
              }}
            >
              ...
              <div className="absolute inset-0 rounded-sm bg-gradient-to-r from-green-400 to-green-500 opacity-0 blur-xl transition-opacity duration-300 group-hover:opacity-100"></div>
