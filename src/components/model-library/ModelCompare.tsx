...
function getModesDisplay(model: ProcessedModel) {
  const modeList = model.data.modes?.length
    ? model.data.modes
    : model.data.supported_modes?.length
      ? model.data.supported_modes
      : null
...
function getContextLength(model: ProcessedModel) {
  return model.data.max_input_tokens || model.data.max_tokens || 0
}
...
export default function ModelCompare({
  ...
  const [leftId, setLeftId] = useState<string | null>(initialLeftId || initialLeftModel?.id || null)
  ...
  const isComparable = leftModel && rightModel ? leftFamily === rightFamily : false
  ...
  const leftModesValue =
    leftModel?.data.modes ?? leftModel?.data.supported_modes ?? leftModel?.data.mode
  ...
  const buildModelFacts = (model: ProcessedModel) => {
    ...
    basicInfo.push(
      `${model.displayName} is a ${getModesDisplay(model).toLowerCase()} model provided by ${formatProviderName(model.provider)}.`
    )

    if (contextLength > 0) {
      if (contextLength >= 1000000) {
        basicInfo.push(
          `This model offers an exceptional context window of ${formatK(contextLength)} tokens, making it ideal for processing extensive documents, long conversations, or large codebases.`
        )
      } else if (contextLength >= 100000) {
        basicInfo.push(
          `With a context window of ${formatK(contextLength)} tokens, this model can handle substantial inputs such as detailed documents or extended conversation histories.`
        )
      } else if (contextLength >= 32000) {
        basicInfo.push(
          `The model supports a ${formatK(contextLength)}-token context window, suitable for moderate-sized documents and multi-turn conversations.`
        )
      } else {
    ...
    if (model.data.input_cost_per_image != null) {
      pricingInfo.push(
        `Image input processing is priced at $${model.data.input_cost_per_image.toFixed(4)} per image.`
      )
    }
    if (model.data.output_cost_per_image != null) {
      pricingInfo.push(
        `Image generation costs $${model.data.output_cost_per_image.toFixed(4)} per image.`
      )
    }
    if (model.data.input_cost_per_second != null) {
      pricingInfo.push(
        `Audio input processing costs $${model.data.input_cost_per_second.toFixed(4)} per second.`
      )
    }
    if (model.data.output_cost_per_second != null) {
      pricingInfo.push(
        `Audio or video output generation costs $${model.data.output_cost_per_second.toFixed(4)} per second.`
      )
    }
    if (model.data.ocr_cost_per_page != null) {
      pricingInfo.push(
        `OCR processing is priced at $${model.data.ocr_cost_per_page.toFixed(4)} per page.`
      )
    }
    ...
    if (model.data.max_output_tokens) {
      limitsInfo.push(
        `The model can generate up to ${formatK(model.data.max_output_tokens)} tokens in a single response.`
      )
    }
    if (model.data.max_query_tokens) {
      limitsInfo.push(
        `Query operations support up to ${formatK(model.data.max_query_tokens)} tokens.`
      )
    }
    ...
    if (model.data.supported_endpoints && model.data.supported_endpoints.length > 0) {
      supportInfo.push(
        `Available through the following endpoints: ${formatList(model.data.supported_endpoints)}.`
      )
    }
    if (model.data.deprecation_date) {
      supportInfo.push(
        `Please note: This model is scheduled for deprecation on ${model.data.deprecation_date}.`
      )
    }
  ...
  const buildComparisonAnalysis = useMemo(() => {
    ...
    if (leftContextValue && rightContextValue && leftContextValue !== rightContextValue) {
      ...
      if (largerValue >= 200000) {
        contextAnalysis +=
          'This substantial difference makes it significantly better suited for processing lengthy documents, extensive codebases, or maintaining long conversation histories.'
      } else if (largerValue >= 100000) {
        contextAnalysis +=
          'This advantage allows for handling larger inputs such as comprehensive reports, extended dialogues, or substantial code repositories.'
      } else {
        contextAnalysis +=
          'This difference may be relevant when working with moderately sized documents or multi-turn conversations.'
      }
    ...
    // Pricing comparison
    if (
      leftInputCostValue != null &&
      rightInputCostValue != null &&
      leftInputCostValue !== rightInputCostValue
    ) {
      ...
      let pricingAnalysis = ''
      if (
        leftModel.data.input_cost_per_token != null ||
        rightModel.data.input_cost_per_token != null
      ) {
      ...
      pricingAnalysis +=
        '. This difference becomes significant in high-volume applications or when processing large amounts of input data.'
    ...
    if (
      leftOutputCostValue != null &&
      rightOutputCostValue != null &&
      leftOutputCostValue !== rightOutputCostValue
    ) {
      ...
      let outputAnalysis = ''
      if (
        leftModel.data.output_cost_per_token != null ||
        rightModel.data.output_cost_per_token != null
      ) {
        const cheaperCostPer1M = (cheaperCost * 1_000_000).toFixed(2)
        const expensiveCostPer1M = (expensiveCost * 1_000_000).toFixed(2)
      ...
      outputAnalysis +=
        '. Consider this when your use case involves generating substantial amounts of text or requires frequent model responses.'
    ...
    if (
      ...
      const smaller =
        leftModel.data.max_output_tokens > rightModel.data.max_output_tokens
          ? rightModel
          : leftModel
      const largerTokens = Math.max(
        leftModel.data.max_output_tokens,
        rightModel.data.max_output_tokens
      )
      const smallerTokens = Math.min(
        leftModel.data.max_output_tokens,
        rightModel.data.max_output_tokens
      )
    ...
    if (leftOnlyCaps.length > 0 || rightOnlyCaps.length > 0) {
      ...
      capabilityAnalysis +=
        '. These distinct capabilities may be decisive factors depending on your application requirements.'
